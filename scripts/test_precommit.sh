...
cd ~/Obsidian/oya-inai-wiki || exit 1
...
echo "=== pre-commit 関所テスト ==="

# --- 関所1: raw/ を -f で強制ステージ ---
git add -f raw/medical/DM-ID-2_Status.md 2>/dev/null
...
check "関所2 remote が存在" block $?
git remote remove testremote 2>/dev/null
git reset -q HEAD -- index.md; git checkout -q -- index.md

# --- 関所3: lint ERROR（機微ゲート違反ページ） ---
mkdir -p wiki/persons
